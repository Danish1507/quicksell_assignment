  ...
  return (
    <div className="kanban-board">
      {Object.keys(groupedTickets).map((group) => {
        ...
        const showRadio = grouping !== 'status'; 
        const showUser = grouping !== 'user';
        ...
        return (
          <div key={group} className="kanban-column">
            ...
            {groupedTickets[group].map(ticket => (
              <TaskCard key={ticket.id} ticket={ticket} showRadio={showRadio} showUser={false} />
            ))}
          </div>
        );
      })}
          ...
          <div className="kanban-column">
            ...
            {groupedTickets['Done'] && groupedTickets['Done'].length > 0 ? (
              groupedTickets['Done'].map(ticket => (
                <TaskCard key={ticket.id} ticket={ticket} showRadio={false} showUser = {true} />
              ))
            ) : (
